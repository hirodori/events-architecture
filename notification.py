...
class Notification:
    ...
    def __create_channel(self):
        ...
        channel.queue_bind(exchange=self.__exchange, queue=queue_name, routing_key=self.__queue2)
        channel.queue_bind(exchange=self.__admin_exchange, queue=queue_name, routing_key=self.__queue3)
...
def callback(ch, method, properties, body):
    if method.routing_key == "estoque_confirmado":
        body_str = body.decode('utf-8')
        body_dict = json.loads(body_str)
        item_id = body_dict['id']
        quantity = body_dict['quantity']
        print(f"Estoque confirmado para {quantity} do item {item_id}!")
    elif method.routing_key == "estoque_insuficiente":
        body_str = body.decode('utf-8')
        body_dict = json.loads(body_str)
        item_id = body_dict['id']
        print(f"Não há estoque suficiente para o item {item_id}!")
    elif method.routing_key == "A":
        body_str = body.decode('utf-8')
        print("Mensagem do ADMIN: " + body_str)


rabitmq_consumer = Notification(callback)
